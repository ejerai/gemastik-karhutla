...
function renderMap(data) {
  ...
  function nearestCity(lat, lon) {
    ...
    for (const [name, clat, clon] of CITIES_ID) {
      const d = Math.hypot(clat - lat, clon - lon);
      if (d < bestDist) { bestDist = d; best = name; }
    }
    if (!best) return "";
    const km = Math.round(bestDist * 111);
    return `${best} (~${km} km)`;
  }
  ...
  riskPoints.forEach(p => {
    ...
    if (!IS_TOUCH) {
      rect.bindTooltip(() => `<b>${Math.round(p.risk_score * 100)}% risiko</b> · ${p.region || "-"}<br>` + `Curah hujan: ${p.precip_mm ?? "-"} mm · ${p.status}<br>` + `Dekat ${nearestCity(p.lat, p.lon)}`, {
    ...
    rect.bindPopup(() => `<div style="font-family:'IBM Plex Mono',monospace; font-size:12px; color:var(--text);">
      <b>${nearestCity(p.lat, p.lon) || "Lokasi tidak diketahui"}</b><br>
      <span style="opacity:.7;">${p.lat.toFixed(2)}, ${p.lon.toFixed(2)} · ${p.region || "-"}</span><br>
      Risiko: <b>${(p.risk_score * 100).toFixed(0)}%</b> · ${p.status}<br>
      Curah hujan hari ini: ${p.precip_mm ?? "-"} mm<br>
      Rata-rata 7 hari: ${p.precip_roll7 ?? "-"} mm</div>`);
    rect.on("popupopen", () => rect.closeTooltip());
...
// dataset 514 kabupaten/kota Indonesia (nama + koordinat pusat)
